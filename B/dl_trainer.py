...
class DLTrainer:
    ...
    def an4_prepare(self):
        ...
        self.train_sampler = train_sampler
        trainloader = AudioDataLoader(self.trainset, num_workers=1, batch_sampler=self.train_sampler)
...
def train_with_single(dnn, dataset, data_dir, nworkers, lr, batch_size, nsteps_update, max_epochs, num_steps=1, num_iter=None, cuda_enable=True):
    if cuda_enable:
    ...
    times = []
    display = 1 if iters_per_epoch > 1 else iters_per_epoch-1
    for epoch in range(max_epochs):
        # todo zhtang ==========
        if dnn == 'lstm':
            hidden = trainer.net.init_hidden()
        if not num_iter is None:
            iters_per_epoch = num_iter
...
if __name__ == '__main__':
    ...
    if args.power_profile:
        ...
        os.system("nohup ./scripts/rocm_power 1>%s 2>&1 &" % pw_logfile)
        time.sleep(30)
    ...
    if args.cuda == 0:
        cuda_enable = False
    train_with_single(args.dnn, args.dataset, args.data_dir, 1, args.lr, args.batch_size, args.nsteps_update, args.max_epochs, args.num_steps, num_iter, cuda_enable=cuda_enable)

    if args.power_profile:
        # kill power profiling
        logger.info("Cool down GPU for 30 secs after executing the task...")
        os.system("killall rocm_power")
        time.sleep(30)
        pass
